...
async fn submit_deploy_in_background(
    ...
    let result = async {
        ...
        let _ = tx.send(SubmissionMessage::StatusUpdate(format!("Building image {} ...", image_tag.trim()))).await;
        docker_build(&dockerfile_abs, &context_path, image_tag.trim()).await?;

        if push {
            let _ = tx.send(SubmissionMessage::StatusUpdate(format!("Pushing image {} ...", image_tag.trim()))).await;
            docker_push(image_tag.trim()).await?;
        }
        ...
        let payload = SubmitJobRequest {
            ...
            image: image_tag.trim().to_string(),
