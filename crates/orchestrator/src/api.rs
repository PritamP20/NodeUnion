...
pub fn build_router(app: AppState) -> Router {
    ...
    .route("/users/:wallet/balance", get(user_balance_handler))
    .route("/users/:wallet/payment-request", post(payment_request_handler))
...
async fn user_balance_handler(
    ...
    match app.stellar.check_user_balance(&wallet).await {
        ...
        Err(e) => {
            eprintln!("[WARN] check_user_balance failed for {}: {}", wallet, e);
...
async fn payment_request_handler(
    ...
    if wallet != payload.wallet {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": "wallet mismatch"})),
        ));
    }

    let networks = networks_repo::list_networks(&app.db)
        .await
        .unwrap_or_default();

    let network_exists = networks.iter().any(|n| n.network_id == payload.network_id);
    if !network_exists {
        return Err((
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "network not found"})),
        ));
    }
    ...
    Ok(Json(serde_json::json!({
        ...
        "network_id": payload.network_id,
        "status": "pending",
        "message": "payment request created - user must approve in dashboard"
    })))
}
...
async fn agent_chunk_status_handler(
    ...
    if let Some(job) = updated_job.clone() {
        ...
        if should_settle_once {
            ...
            if let Err(err) = app
                .stellar
                .record_usage_on_chain(&payload.job_id, u128::from(units_for_chain))
                .await
            {
                eprintln!(
                    "[STELLAR] Failed to record usage for job {}: {}",
                    payload.job_id, err
                );
            }

            if let Ok(tx_sig) = app.stellar.close_escrow_on_chain(&payload.job_id).await {
                println!("[STELLAR] Closed escrow for job {}: {}", payload.job_id, tx_sig);

                // Fetch the network's actual price per unit from on-chain
                let price_per_unit = app
                    .stellar
                    .get_network_price_per_unit()
                    .await
                ...
                let amount_tokens = units_metered * price_per_unit;
                ...
                eprintln!("[STELLAR] Failed to close escrow for job {}", payload.job_id);
            }
        }
    }
...
async fn dispatch_job_to_idle_node(app: &AppState, job_id: &str) -> Result<(), ()> {
    ...
    for candidate_node_id in candidate_node_ids {
        ...
        if let Some(_provider_wallet) = &node.provider_wallet {
            ...
            let deposit_amount = u128::from(estimated_units.saturating_mul(100));
            let _ = app
                .stellar
                .open_escrow_on_chain(
                    job_id,
                    u128::from(estimated_units),
                    deposit_amount,
                    &escrow_wallet,
                )
                .await;
